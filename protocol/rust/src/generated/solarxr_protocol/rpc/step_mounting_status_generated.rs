// automatically generated by the FlatBuffers compiler, do not modify
// @generated
extern crate alloc;
extern crate flatbuffers;
use alloc::boxed::Box;
use alloc::string::{String, ToString};
use alloc::vec::Vec;
use core::mem;
use core::cmp::Ordering;
use self::flatbuffers::{EndianScalar, Follow};
use super::*;
#[deprecated(since = "2.0.0", note = "Use associated constants instead. This will no longer be generated in 2021.")]
pub const ENUM_MIN_STEP_MOUNTING_STATUS: u8 = 0;
#[deprecated(since = "2.0.0", note = "Use associated constants instead. This will no longer be generated in 2021.")]
pub const ENUM_MAX_STEP_MOUNTING_STATUS: u8 = 6;
#[deprecated(since = "2.0.0", note = "Use associated constants instead. This will no longer be generated in 2021.")]
#[allow(non_camel_case_types)]
pub const ENUM_VALUES_STEP_MOUNTING_STATUS: [StepMountingStatus; 7] = [
  StepMountingStatus::NONE,
  StepMountingStatus::WAITING_FOR_MOVEMENT,
  StepMountingStatus::WAITING_FOR_REST,
  StepMountingStatus::PROCESSING,
  StepMountingStatus::DONE,
  StepMountingStatus::ERROR_TIMEOUT,
  StepMountingStatus::ERROR_HIGH_ERROR,
];

#[derive(Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Default)]
#[repr(transparent)]
pub struct StepMountingStatus(pub u8);
#[allow(non_upper_case_globals)]
impl StepMountingStatus {
  pub const NONE: Self = Self(0);
  pub const WAITING_FOR_MOVEMENT: Self = Self(1);
  pub const WAITING_FOR_REST: Self = Self(2);
  pub const PROCESSING: Self = Self(3);
  pub const DONE: Self = Self(4);
  pub const ERROR_TIMEOUT: Self = Self(5);
  pub const ERROR_HIGH_ERROR: Self = Self(6);

  pub const ENUM_MIN: u8 = 0;
  pub const ENUM_MAX: u8 = 6;
  pub const ENUM_VALUES: &'static [Self] = &[
    Self::NONE,
    Self::WAITING_FOR_MOVEMENT,
    Self::WAITING_FOR_REST,
    Self::PROCESSING,
    Self::DONE,
    Self::ERROR_TIMEOUT,
    Self::ERROR_HIGH_ERROR,
  ];
  /// Returns the variant's name or "" if unknown.
  pub fn variant_name(self) -> Option<&'static str> {
    match self {
      Self::NONE => Some("NONE"),
      Self::WAITING_FOR_MOVEMENT => Some("WAITING_FOR_MOVEMENT"),
      Self::WAITING_FOR_REST => Some("WAITING_FOR_REST"),
      Self::PROCESSING => Some("PROCESSING"),
      Self::DONE => Some("DONE"),
      Self::ERROR_TIMEOUT => Some("ERROR_TIMEOUT"),
      Self::ERROR_HIGH_ERROR => Some("ERROR_HIGH_ERROR"),
      _ => None,
    }
  }
}
impl core::fmt::Debug for StepMountingStatus {
  fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
    if let Some(name) = self.variant_name() {
      f.write_str(name)
    } else {
      f.write_fmt(format_args!("<UNKNOWN {:?}>", self.0))
    }
  }
}
impl<'a> flatbuffers::Follow<'a> for StepMountingStatus {
  type Inner = Self;
  #[inline]
  unsafe fn follow(buf: &'a [u8], loc: usize) -> Self::Inner {
    let b = flatbuffers::read_scalar_at::<u8>(buf, loc);
    Self(b)
  }
}

impl flatbuffers::Push for StepMountingStatus {
    type Output = StepMountingStatus;
    #[inline]
    unsafe fn push(&self, dst: &mut [u8], _written_len: usize) {
        flatbuffers::emplace_scalar::<u8>(dst, self.0);
    }
}

impl flatbuffers::EndianScalar for StepMountingStatus {
  type Scalar = u8;
  #[inline]
  fn to_little_endian(self) -> u8 {
    self.0.to_le()
  }
  #[inline]
  #[allow(clippy::wrong_self_convention)]
  fn from_little_endian(v: u8) -> Self {
    let b = u8::from_le(v);
    Self(b)
  }
}

impl<'a> flatbuffers::Verifiable for StepMountingStatus {
  #[inline]
  fn run_verifier(
    v: &mut flatbuffers::Verifier, pos: usize
  ) -> Result<(), flatbuffers::InvalidFlatbuffer> {
    use self::flatbuffers::Verifiable;
    u8::run_verifier(v, pos)
  }
}

impl flatbuffers::SimpleToVerifyInSlice for StepMountingStatus {}
